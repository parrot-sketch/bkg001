...
export class TestUserFactory {
  ...
  static async createPatientUser(overrides?: Partial<{
    email: string;
    firstName: string;
    lastName: string;
    password?: string;
...
export class MockUserRepository {
  ...
  private findByIdsMock = vi.fn();
  private findByRoleMock = vi.fn();
  private updateEmailMock = vi.fn();
  private updatePasswordMock = vi.fn();
  ...
  async findByIds(ids: string[]): Promise<User[]> {
    return this.findByIdsMock(ids);
  }

  async findByRole(role: string): Promise<User[]> {
    return this.findByRoleMock(role);
  }
  ...
  async updateEmail(userId: string, newEmail: string): Promise<void> {
    return this.updateEmailMock(userId, newEmail);
  }

  async updatePassword(userId: string, hashedPassword: string): Promise<void> {
    return this.updatePasswordMock(userId, hashedPassword);
  }
  ...
  getMocks() {
    return {
      ...
      findByIds: this.findByIdsMock,
      findByRole: this.findByRoleMock,
      updateEmail: this.updateEmailMock,
      updatePassword: this.updatePasswordMock,
    };
  }
  ...
  reset(): void {
    ...
    this.findByIdsMock.mockReset();
    this.findByRoleMock.mockReset();
    this.updateEmailMock.mockReset();
    this.updatePasswordMock.mockReset();
  }
}
