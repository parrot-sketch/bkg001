# Resume Session Runtime Validation

## Executive Summary

This document validates the runtime behavior of the `resumeSession` Server Action. The execution is traced, counted, and verified for correctness.
...
| State | Value |
|-------|-------|
| `session.workflowState` | IDLE or PAUSED |
...
IDLE/PAUSED
  → WorkflowEngine.execute({ type: 'START_CONSULTATION' })
    ...
    → State transition: IDLE/PAUSED → ACTIVE
...
### 3.3 Consultation Not Found

| Step | Expected | Status |
|------|----------|--------|
| SessionService validates | consultation not found | ✅ |
| Returns error | SESSION_NOT_FOUND | ✅ |
| Factory throws | Error propagated | ✅ |
| Server Action catches | Returns UNKNOWN error | ✅ |
| UI shows error | → toast.error() | ✅ |

### 3.4 Consultation Not In Progress
