/**
 * API Route: POST /api/doctor/consents/templates/[templateId]/approve
 * 
...
import { parseApproveTemplate } from '@/lib/parsers/consentPhase2Parsers';
...
export async function POST(
    ...
    try {
        ...
        if (authResult.user.role !== Role.ADMIN) {
            return handleApiError(new ForbiddenError('Only administrators can approve templates'));
        }

        const { templateId } = await context.params;
        const body = await request.json().catch(() => ({}));
        const parsed = parseApproveTemplate({ ...body, templateId });

        const service = getConsentTemplateService();

        const updated = await service.approveTemplate(templateId, {
        ...
        console.error('[API] POST /api/doctor/consents/templates/[templateId]/approve - Error:', error);
