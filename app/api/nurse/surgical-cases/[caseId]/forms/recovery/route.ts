 ...
 *
 * GET  /api/nurse/surgical-cases/[caseId]/forms/recovery
...
function mapResponseDto(response: {
    ...
    return {
        ...
        data,
        sectionCompletion: getRecoverySectionCompletion(data),
...
export async function GET(
    ...
    try {
        ...
        const timer = endpointTimer('GET /api/nurse/forms/recovery');
        ...
        if (!response) {
            if (auth.user.role !== Role.NURSE) {
                return NextResponse.json({
                    success: true,
                    data: null,
                    message: 'No recovery record started yet',
                });
            }

            const template = await db.clinicalFormTemplate.findFirst({
                where: { key: RECOVERY_TEMPLATE_KEY, version: RECOVERY_TEMPLATE_VERSION, is_active: true },
            });
            if (!template) {
                return NextResponse.json(
                    { success: false, error: 'Recovery record template not found. Contact admin.' },
                    { status: 500 },
                );
            }
        ...
        console.error('[API] GET recovery form error:', error);
...
export async function PUT(
    ...
    try {
        ...
        if (!existing) {
            return NextResponse.json(
                { success: false, error: 'No recovery record draft found. Open the form first.' },
                { status: 404 },
            );
        }
        ...
        if (existing.status === ClinicalFormStatus.FINAL) {
            return NextResponse.json(
                { success: false, error: 'Recovery record is finalized and locked.' },
                { status: 409 },
            );
        }

        const body = await request.json();
        const parsed = nurseRecoveryRecordDraftSchema.safeParse(body.data);
        if (!parsed.success) {
            return NextResponse.json(
                {
                    success: false,
        ...
        await db.clinicalAuditEvent.create({
            data: {
                ...
                action_type: 'RECOVERY_RECORD_DRAFT_SAVED',
        ...
        console.error('[API] PUT recovery form error:', error);
        return NextResponse.json({ success: false, error: 'Internal server error' }, { status: 500 });
    }
}
