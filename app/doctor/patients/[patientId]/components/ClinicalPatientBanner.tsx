    ...
    <div className="bg-white border border-slate-200 shadow-sm p-6 space-y-5 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">{patientName}</h1>
            <span className="px-2 py-0.5 text-[10px] font-semibold bg-slate-100 text-slate-700 uppercase tracking-wider font-mono border border-slate-200 rounded">
        ...
        <div className="w-full md:max-w-md shrink-0">
          {patient.allergies ? (
            ...
            <div className="flex items-center gap-2.5 bg-emerald-50 border-l-4 border-emerald-500 p-3 text-emerald-900 rounded-r-md">
              ...
              <div className="text-xs font-semibold">No known drug or food allergies</div>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-slate-100 text-xs">
        <div>
          ...
          <span className="font-semibold text-slate-800 font-mono">{dobFormatted}</span>
        </div>
        <div>
          <span className="block text-[10px] uppercase font-bold text-slate-400 tracking-wider">Age / Sex</span>
          <span className="font-semibold text-slate-800">
            {patient.dateOfBirth ? `${calculateAge(patient.dateOfBirth)} yrs` : `${patient.age} yrs`} · {patient.gender}
          </span>
        </div>
        <div>
          <span className="block text-[10px] uppercase font-bold text-slate-400 tracking-wider">Blood Group</span>
          <span className="font-semibold text-slate-850 font-mono text-blue-700">
        ...
        <div className="flex items-end justify-start sm:justify-end">
          <Button
            ...
            className="h-8 text-xs font-medium text-slate-500 hover:text-slate-900 p-0 hover:bg-transparent"
          >
            {showAdmin ? (
              <span className="flex items-center gap-1">Hide Admin Details <ChevronUp className="h-3 w-3" /></span>
            ) : (
              <span className="flex items-center gap-1">Show Admin Details <ChevronDown className="h-3 w-3" /></span>
            )}
          </Button>
        </div>
      </div>
