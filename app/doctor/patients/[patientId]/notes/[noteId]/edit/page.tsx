...
export default function EditClinicalNotePage() {
    ...
    const loadNote = async () => {
      try {
        ...
        if (response.success && response.data) {
          ...
          setNoteType(response.data.noteType);
          setContent(response.data.content);
        } else if (!response.success) {
          toast.error(response.error || 'Failed to load note');
        }
      } catch {
  ...
  if (authLoading || patientLoading || noteLoading || !user || !isAuthenticated) {
    return (
  ...
  if (!patient || !note) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-8">
        <p className="text-slate-500">Patient or note not found</p>
      </div>
    );
  }
