...
import { ConsultationQueuePanel } from '@/components/consultation/ConsultationQueuePanel';
...
export default function ConsultationSessionPage() {
  ...
  // Fetch today's appointments for queue panel
  ...
  const handleStartConsultation = useCallback(() => {
    if (!appointment ||
      ...
      toast.error('Appointment status invalid for starting consultation');
      return;
    }
  ...
  // Auto-trigger start dialog if not started and appointment is ready
  // This handles the flow where doctor clicks "Start" from dashboard
  // eslint-disable-next-line react-hooks/rules-of-hooks
  useEffect(() => {
    if (isNotStarted && appointment && !showStartDialog) {
  ...
  }, [isNotStarted, appointment, showStartDialog]);
  ...
  return (
    <div className="flex flex-col h-screen bg-background">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* Right: Waiting Queue Panel */}
        <ConsultationQueuePanel
          currentAppointmentId={appointmentId ?? undefined}
          appointments={todayAppointments}
          onSwitchPatient={(newAppointmentId) => {
            router.push(`/doctor/consultations/${newAppointmentId}/session`);
          }}
