...
interface FrontdeskAppointmentDetailsCardProps {
...
export function FrontdeskAppointmentDetailsCard({
  appointmentDate,
  time,
  type,
  doctorName,
...
}: FrontdeskAppointmentDetailsCardProps) {
    ...
    <div className="border border-[#e7d6bf] bg-white">
      ...
      <div className="p-4 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p className="text-[10px] uppercase tracking-wider font-semibold text-[#2c2e4b]/60">Date</p>
            <p className="text-sm text-[#2c2e4b] font-medium">{formatDate(appointmentDate)}</p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-wider font-semibold text-[#2c2e4b]/60">Time</p>
            <p className="text-sm text-[#2c2e4b] font-medium flex items-center gap-1">
              <ClockIcon className="h-4 w-4 text-[#caa26a]" />
              {time || 'Not specified'}
            </p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-wider font-semibold text-[#2c2e4b]/60">Type</p>
            <p className="text-sm text-[#2c2e4b] font-medium capitalize">{type || 'Consultation'}</p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-wider font-semibold text-[#2c2e4b]/60">Doctor</p>
            <p className="text-sm text-[#2c2e4b] font-medium flex items-center gap-1">
              <StethoscopeIcon className="h-4 w-4 text-[#caa26a]" />
              {doctorName || 'Not assigned'}
            </p>
          </div>
        </div>

        {(checkedInAt || consultationStartedAt || consultationEndedAt || consultationDuration) && (
          <div className="border-t border-[#e7d6bf]/60 pt-4 space-y-3">
            {checkedInAt && (
              <div className="flex items-center gap-2.5 text-sm text-[#2c2e4b]/70">
                <UserCheckIcon className="h-4 w-4 text-[#caa26a] shrink-0" />
                <span className="text-xs uppercase tracking-wider font-semibold text-[#2c2e4b]/60 mr-1">Checked In:</span>
                <span className="text-[#2c2e4b] font-medium">{format(new Date(checkedInAt), 'h:mm a')}</span>
              </div>
            )}
            {consultationStartedAt && (
              <div className="flex items-center gap-2.5 text-sm text-[#2c2e4b]/70">
                <ActivityIcon className="h-4 w-4 text-[#caa26a] shrink-0" />
                <span className="text-xs uppercase tracking-wider font-semibold text-[#2c2e4b]/60 mr-1">Consult Started:</span>
                <span className="text-[#2c2e4b] font-medium">{format(new Date(consultationStartedAt), 'h:mm a')}</span>
              </div>
            )}
            {consultationEndedAt && (
              <div className="flex items-center gap-2.5 text-sm text-[#2c2e4b]/70">
                <ClipboardCheckIcon className="h-4 w-4 text-[#caa26a] shrink-0" />
                <span className="text-xs uppercase tracking-wider font-semibold text-[#2c2e4b]/60 mr-1">Consult Ended:</span>
                <span className="text-[#2c2e4b] font-medium">{format(new Date(consultationEndedAt), 'h:mm a')}</span>
              </div>
            )}
            {consultationDuration && (
              <div className="flex items-center gap-2.5 text-sm text-[#2c2e4b]/70">
                <ClockIcon className="h-4 w-4 text-[#caa26a] shrink-0" />
                <span className="text-xs uppercase tracking-wider font-semibold text-[#2c2e4b]/60 mr-1">Duration:</span>
                <span className="text-[#2c2e4b] font-medium">{consultationDuration} min</span>
              </div>
            )}
          </div>
        )}
