/**
 * Clean Data Seed Script for Production
 * 
 * Uses accelerate to seed clean Services and Inventory to production DB.
 * 
 * Run with: node prisma/seed-production.cjs
 */
...
async function seedServices(catalogueServices, procedureServices) {
  ...
  console.log(`Seeding ${allServices.length} services to production...`);
  ...
  for (let i = 0; i < allServices.length; i += chunkSize) {
    ...
    console.log(`  Inserted ${Math.min(i + chunkSize, allServices.length)}/${allServices.length}`);
  }
...
async function seedInventory(inventoryItems) {
  console.log(`Seeding ${inventoryItems.length} inventory items to production...`);
  
  const deleteResult = await prisma.inventoryItem.deleteMany({});
  console.log(`  Deleted ${deleteResult.count} existing inventory items`);
  ...
  for (let i = 0; i < inventoryItems.length; i += chunkSize) {
    ...
    console.log(`  Inserted ${Math.min(i + chunkSize, inventoryItems.length)}/${inventoryItems.length}`);
  }
