-- Fix: Create surgical_procedure_options table if missing
-- This is needed because migration 20260412000000 assumes the table exists

-- Check if surgical_procedure_options exists, if not create it
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'surgical_procedure_options') THEN
        CREATE TABLE "surgical_procedure_options" (
            ...
            "code" VARCHAR(255),
            "category" "ProcedureCategory" DEFAULT 'GENERAL',
            "description" TEXT,
        ...
        CREATE INDEX IF NOT EXISTS "surgical_procedure_options_code_idx" ON "surgical_procedure_options"("code");
        CREATE INDEX IF NOT EXISTS "surgical_procedure_options_category_idx" ON "surgical_procedure_options"("category");
    END IF;
END $$;

-- Now add columns if they don't exist (safer approach)
DO $$ 
BEGIN
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'procedure_category_config') THEN
        CREATE TABLE "procedure_category_config" (
            "id" VARCHAR(255) NOT NULL DEFAULT cuid(),
            "name" VARCHAR(255) NOT NULL,
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'procedure_subcategory') THEN
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'procedure_service_link') THEN
        CREATE TABLE "procedure_service_link" (
            "id" VARCHAR(255) NOT NULL DEFAULT cuid(),
...
-- Create ProcedureStatus type if needed
DO $$ 
BEGIN
