...
/**
 * Type-first staff field — free-text name with optional directory suggestions.
 */
...
function displayFromValue(value: string, options: StaffUserOption[]): string {
  if (!value) return '';
  if (value.startsWith('__custom__:')) return value.replace('__custom__:', '');
  return options.find((o) => o.id === value)?.fullName ?? '';
}
...
export function StaffCombobox({
  ...
  const exactMatch = useMemo(() => {
    ...
    return options.find((o) => o.fullName.toLowerCase() === q);
  }, [options, query]);
  ...
  const commitOption = (option: StaffUserOption) => {
    onChange(option.id);
  ...
  const commitCurrent = () => {
    ...
    if (query.trim()) commitCustom(query);
  };
  ...
  const showUseTyped = query.trim().length > 0 && !exactMatch;
  ...
  return (
    <div ref={rootRef} className="relative">
          ...
          {filtered.map((option, idx) => (
            ...
            >
              ...
              <span className="min-w-0">
                <span className="block font-medium text-slate-800">{option.fullName}</span>
      ...
      {roleHint ? <p className="mt-1 text-[11px] text-slate-500">{roleHint}</p> : null}
    </div>
  );
}
