          ...
          {(['planning', 'scheduled', 'recovery'] as TabValue[]).map((tab) => (
            <TabsContent key={tab} value={tab} className="mt-0 px-5 py-4">
              {isLoading ? (
                ...
                <div className="py-10 text-center text-sm text-[#2c2e4b]/60">No {tab} cases</div>
              ) : (
                <div className="space-y-2">
                  {filteredCases.map((caseItem) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <Badge variant="outline" className="text-[10px] font-medium border-[#2c2e4b]/10 text-[#2c2e4b]/70 shrink-0">
                          {caseItem.status}
                        </Badge>
                      </div>
                    </div>
